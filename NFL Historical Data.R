...
## kickoff recovery statistics (return for td and whether returning team recovered kickoff)
## potential need for lateral information
## potential need for accounting for fumbles on interceptions and stupid stuff like that
## wpa & epa 
    ## success: whether epa > 0 for a given play
## strength of schedule


pbp <- nflfastR::load_pbp(2003:2022)
...
pbp_agg_all <- bind_rows(pbp_agg_home, pbp_agg_away) %>% 
...
year_week_team <- lapply(unique(pbp_agg_all$season), function(y) {
  expand.grid(
    season = y,
    week = unique(pbp_agg_all$week),
    team = unique(pbp_agg_all$team)
  )
}) %>% 
  bind_rows() %>% 
  filter((season <= 2020 & week <= 21) | (season >= 2021 & week <= 22))

full_epa_wpa_rank <- year_week_team %>% 
left_join(pbp_agg_all, by = c("season", "week", "team")) %>% 
group_by(team) %>%
...
full_data <- pbp_game_detail %>% 
  left_join(full_epa_wpa_rank, by = c("season" = "season", "week" = "week", "home_team" = "team")) %>% 
  #filter(week != 1) %>% 
  ...
  left_join(full_epa_wpa_rank, by = c("season" = "season", "week" = "week", "away_team" = "team")) %>% 
  filter(week != 1) %>% 
  select (-season_type) %>% 
  rename(
  ...
  select(-game_id, -old_game_id, -home_team, -away_team, -home_score, -away_score, -total, -start_time, -game_stadium) %>% 
...
test_data <- colSums(is.na(full_data))
rm(test_data)

## maybe include week 1 for just game description model, then exclude it for just team ranking model and
## combined model

                        ## add differences between predicted vs actual spread
                        
                        ## add differences between different ranks for home vs away teams

## explore how 1.5, 2, 2.5, 3 point home-field advantage could affect spreads?

## exact difference in expected vs actual betting metrics vs categorical (equal, home team spread win, 
## away team spread win) or (over/under)

## look at avg difference between betting metrics throughout the years (has Vegas become better at predicting?)
      ## this could maybe mean including year vs not including year in the model maybe?
